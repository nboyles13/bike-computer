    ...
    public String format(OsmTrack t) {
        ...
        if (t.exportWaypoints || t.exportCorrectedWaypoints || !t.pois.isEmpty()) {
            ...
            if (t.exportCorrectedWaypoints) {
                ...
                for (int i2 = 0; i2 < t.matchedWaypoints.size(); i2++) {
                    MatchedWaypoint wp = t.matchedWaypoints.get(i2);
                    if (wp.correctedpoint != null) {
                        OsmNodeNamed n2 = new OsmNodeNamed(wp.correctedpoint);
                        n2.name = wp.name + "_corr";
                        list.add(n2);
                    }
                }
                createViaFolder(sb, "via_corr", list.subList(0, list.size()));
            }
        }
        sb.append("  </Document>\n");
        sb.append("</kml>\n");
