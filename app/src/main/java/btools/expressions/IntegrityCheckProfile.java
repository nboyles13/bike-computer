...
public class IntegrityCheckProfile {
    ...
    public void integrityTestProfiles(File lookupFile, File profileDir) {
        ...
        if (files == null) {
            System.err.println("no files " + String.valueOf(profileDir));
            return;
        }
        if (!lookupFile.exists()) {
            System.err.println("no lookup file " + String.valueOf(lookupFile));
            return;
        }
        for (File f : files) {
            if (f.getName().endsWith(".brf")) {
                ...
                System.out.println("test " + ((int) meta.lookupVersion) + "." + ((int) meta.lookupMinorVersion) + " " + String.valueOf(f));
            }
        }
    }
}
